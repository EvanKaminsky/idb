...
export default class CountriesPage extends React.Component {
    ...
    reload(page, filterSortString) {
        ...
        window.constants.api.search("countries", page, this.state.page_size, null, filterSortString).then(json => {
            this.relayout(json);
        });
    };
    ...
    setDescriptors(json) {
        if (this.state.descriptors === null && json !== null && json.country_fields !== null) {
            this.setState({descriptors: json.country_fields.map(field => field.Field)});
        }
    }
    ...
    render() {
        ...
        return (
            <div>
                <h1>Tipsy Mix</h1>
                <TipsySearchbar category={"countries"} placeholder="Search for countries" spin={this.spin} relayout={this.relayout}/>
                {filtersort}
